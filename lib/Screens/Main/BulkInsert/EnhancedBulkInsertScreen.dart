...
class EnhancedBulkInsertScreen extends StatefulWidget {
  const EnhancedBulkInsertScreen({super.key});

  @override
  _EnhancedBulkInsertScreenState createState() =>
      _EnhancedBulkInsertScreenState();
}

class _EnhancedBulkInsertScreenState extends State<EnhancedBulkInsertScreen> {
  ...
  Future<void> _showInfoDialog() async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('bulkInsertScreen.infoTitle'.tr()),
          content: Text('bulkInsertScreen.infoMessage'.tr()),
          actions: <Widget>[
            TextButton(
              child: Text('bulkInsertScreen.ok'.tr()),
  ...
  void _filterParties(String query) {
    setState(() {
      // This will trigger a rebuild with filtered results in the ListView
    });
  }
  ...
  void _processBulkUpdate() async {
    ...
    final canProceed = await PremiumUtils.checkPremiumForOperation(
      context,
      operationName: 'Bulk Update and SMS',
    );

    if (!canProceed) {
      return; // Premium check failed, don't proceed
    }
    ...
    try {
      ...
      for (var entry in entries) {
        try {
          ...
          // Use existing CollectionScreen methods
  ...
  Future<void> _prepareSmsForParty(Map<String, dynamic> party, double amount,
    ...
    if (sms == 1 && phone.isNotEmpty && phone != 'Unknown') {
      ...
      // Compact SMS message with date but no time
    ...
    for (int i = 0; i < smsQueue.length; i++) {
      ...
      if (sendThis == true) {
        try {
          ...
          if (mounted) {
            _showSnackBar(
                'bulkInsertScreen.smsSent'
                    .tr(namedArgs: {'partyName': smsData['partyName']!}),
                Colors.green);
          }
        } catch (e) {
          if (mounted) {
            _showSnackBar(
                'bulkInsertScreen.smsFailed'
                    .tr(namedArgs: {'partyName': smsData['partyName']!}),
                Colors.red);
          }
        }
        ...
        return AlertDialog(
          content: Column(
            ...
            children: [
              ...
              Text('bulkInsertScreen.processingCollections'
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('bulkInsertScreen.successfullyUpdated'
                  .tr(namedArgs: {'count': count.toString()})),
              Text('bulkInsertScreen.totalAmount'
                  .tr(namedArgs: {'amount': total.toStringAsFixed(2)})),
              Text('bulkInsertScreen.smsProcessed'
                  .tr(namedArgs: {'count': smsCount.toString()})),
            ],
          ),
          actions: [
            ElevatedButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('bulkInsertScreen.collectionEntry'.tr(),
            style: const TextStyle(
            ...
            children: [
                  ...
                  child: Column(
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'bulkInsertScreen.searchParty'.tr(),
                          border: const OutlineInputBorder(),
                          isDense: true,
                          contentPadding: const EdgeInsets.symmetric(
                              vertical: 8, horizontal: 10),
              ...
              Expanded(
                child: filteredLendingDetails.isEmpty
                    ? Center(
                        child: Text(
                          'bulkInsertScreen.noPartiesFound'.tr(),
                          textAlign: TextAlign.center,
                          style:
